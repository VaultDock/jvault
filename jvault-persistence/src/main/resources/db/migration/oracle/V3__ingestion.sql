-- jvault Kafka ingestion state. Oracle.
--
...
CREATE TABLE ingested_message (
    ...
    message_offset  NUMBER(19)       NOT NULL,
    mapping_id      VARCHAR2(128)  NOT NULL,
    dedupe_key      VARCHAR2(200),
    correlation_id  VARCHAR2(200)  NOT NULL,
...
-- Oracle has no partial index; a key that is entirely NULL is simply not indexed, so nulling
-- the whole key when there is no dedupe key reproduces the filtered index exactly.
CREATE UNIQUE INDEX uq_ingested_dedupe ON ingested_message (
    CASE WHEN dedupe_key IS NULL THEN NULL ELSE mapping_id END,
    dedupe_key);
