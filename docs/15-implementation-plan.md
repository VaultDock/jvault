...
| **Streaming encryption** | Does the chosen library (AWS Encryption SDK vs Tink) stream 1 GB with bounded heap and support the partial decryption our range reads need? | If range decryption is awkward: ranged reads decrypt-and-discard. Costs throughput, not correctness (§9.3) |
| **Jira sandbox conformance** | Against a real Cloud sandbox **and** a DC container: does `POST /issue` accept `properties` at create time? What do createmeta responses look like for the customer's actual custom field types, on both? What are the observed rate-limit behaviours? | Confirms or corrects §0.9, §2.3 and the rate-limiter tuning |
